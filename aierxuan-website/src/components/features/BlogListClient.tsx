...
function getBlogCoverImage(src?: string | null) {
    if (!src || src.startsWith('/uploads/blog/')) {
        return BLOG_CARD_FALLBACK_IMAGE
    }
...
function BlogPostCard({
    ...
    const [coverImage, setCoverImage] = useState(getBlogCoverImage(post.cover_image))

    useEffect(() => {
        setCoverImage(getBlogCoverImage(post.cover_image))
    }, [post.cover_image])
        ...
        <article className="group bg-white/5 backdrop-blur-sm rounded-2xl overflow-hidden hover:bg-white/10 transition-all duration-300 border border-white/10 hover:border-blue-500/30">
            <div className="aspect-video bg-slate-800 relative overflow-hidden">
                <Image
                    ...
                    onError={() => setCoverImage(BLOG_CARD_FALLBACK_IMAGE)}
                />
            </div>
            <div className="p-6">
